...
from common.frozenmodel import labels


def generate_model(*, weights_filepath: Optional[str], with_augmentations: bool = False, can_ignore_weights: bool = False) -> tf.keras.Model:
    shape = (1080, 1920, 3)
...
def to_tflite(model: tf.keras.Model) -> bytes:
    ...
    if isinstance(tflite_model, bytes):
        return tflite_model
    raise Exception('Exported model is not serialized')
