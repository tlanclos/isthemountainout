...
def generate_model(*, weights_filepath: Optional[str] = None) -> tf.keras.Model:
    ...
    outputs = m.chained(
        ...
        m.expand(
            flow=lambda previous_activation, size: tf.keras.layers.add([
                ...
                tf.keras.layers.Conv2D(filters=size, kernel_size=1, strides=2, padding='same')(
                    previous_activation),
            ]),
            values=[128, 256, 728],
        ),
        ...
        lambda previous_activation: tf.keras.layers.add([
            ...
            tf.keras.layers.Conv2D(filters=1024, kernel_size=1, strides=2, padding='same')(
                previous_activation),
        ]),
        tf.keras.layers.Activation('relu'),
        tf.keras.layers.SeparableConv2D(
    ...
    model.load_weights(
        weights_filepath if weights_filepath else 'isthemountainout.h5')
    return model
